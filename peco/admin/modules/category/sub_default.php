...
function default_default(){
	...
	$clsDataGrid->setTitle2($core->getLang("Category_Management"));
	...
	$clsDataGrid->addColumnLabel("short_title", "ShortTitle", "width='10%' align='center'");
	$clsDataGrid->addColumnLabel("thumbnail", "Thumbnail", "width='5%' align='center'");
	...
	$clsDataGrid->addColumnLabel("disabled", "Disabled", "width='5%' align='center'");
...
function default_add(){
	...
	$id = GET("id", 0);
	$arrOneCategory = array();
	if ($id>0){
		$arrOneCategory = $clsCategories->getOne($id);		
	}
	$btnSave = POST("btnSave", "");
	$errors = array();
	$valid = 1;
	if ($btnSave!=""){
		$valid = $clsCategories->checkValidAddForm($errors);
		if ($valid){
			$valid = $clsCategories->doAddRecord($errors);		
			if ($valid){
				redirectURL("?mod=$mod");
				exit();
			}
	...
	if (is_array($arrOneCategory)){
		foreach ($arrOneCategory as $key => $val) {$_POST[$key] = $val;}
		$_POST["old_short_title"] = isset($arrOneCategory["short_title"])? $arrOneCategory["short_title"] : "";
		$_POST["old_thumbnail"] = isset($arrOneCategory["thumbnail"])? $arrOneCategory["thumbnail"] : "";
	}
